  ...
  return (
    ...
    <Card className="shrink-0 w-[280px] sm:w-[320px] md:w-[350px] lg:w-[380px] bg-linear-to-br from-slate-700/60 to-slate-800/60 border-white/10 backdrop-blur-xl hover:border-orange-500/50 transition-all duration-300 hover:shadow-xl hover:shadow-orange-500/20 group overflow-hidden snap-start">
      <CardContent className="p-0">
        {/* Room Image Carousel */}
        {roomImages.length > 0 ? (
          <div className="relative h-40 w-full overflow-hidden group/carousel">
            {!imageError ? (
              <Image
              ...
              <div className="absolute inset-0 bg-gradient-to-br from-slate-700/80 to-slate-800/80 flex flex-col items-center justify-center">
                ...
                <p className="text-slate-400 text-xs font-medium">Failed to load image</p>
              </div>
            )}
            ...
            {roomImages.length > 1 && (
              <>
                <Button
                  variant="ghost"
                  size="sm"
                  className="absolute left-1.5 top-1/2 -translate-y-1/2 bg-black/60 backdrop-blur-md text-white hover:bg-orange-500 z-10 transition-all duration-300 border border-white/10 h-7 w-7 p-0 rounded-full opacity-0 group-hover/carousel:opacity-100"
                ...
                <Button
                  variant="ghost"
                  size="sm"
                  className="absolute right-1.5 top-1/2 -translate-y-1/2 bg-black/60 backdrop-blur-md text-white hover:bg-orange-500 z-10 transition-all duration-300 border border-white/10 h-7 w-7 p-0 rounded-full opacity-0 group-hover/carousel:opacity-100"
                  ...
                  <button
                    ...
                    className={`h-1 rounded-full transition-all duration-300 ${
                      idx === currentImageIndex 
                        ? 'bg-orange-500 w-3 shadow-lg shadow-orange-500/50' 
                        : 'bg-white/40 hover:bg-white/70 w-1'
                    }`}
          ...
          <div className="relative h-40 w-full bg-gradient-to-br from-slate-700/80 to-slate-800/80 flex flex-col items-center justify-center">
            <ImageIcon className="w-10 h-10 text-slate-500 mb-1.5" />
            <p className="text-slate-400 text-xs font-medium">No images available</p>
          </div>
        )}
        
        <div className="p-3 sm:p-3.5">
          <div className="mb-3">
            <h4 className="font-bold text-white text-base sm:text-lg mb-1.5">{room.name}</h4>
            <div className="flex items-center gap-1.5 flex-wrap">
              <Badge className="bg-orange-500/20 text-orange-500 border-orange-500/30 text-xs px-2 py-0.5">{room.category}</Badge>
              {room.ac && <Badge className="bg-blue-500/20 text-blue-400 border-blue-500/30 text-xs px-2 py-0.5">AC</Badge>}
              {room.attachedWashroom && <Badge className="bg-purple-500/20 text-purple-400 border-purple-500/30 text-xs px-2 py-0.5">Attached Bathroom</Badge>}
              {room.balcony && <Badge className="bg-green-500/20 text-green-400 border-green-500/30 text-xs px-2 py-0.5">Balcony</Badge>}
            </div>
          </div>
          ...
          <div className="space-y-1.5 mb-3">
            <div className="flex justify-between items-center">
              ...
              <span className="text-white font-semibold text-sm">{room.roomSize}</span>
            </div>
             <div className="flex justify-between items-center">
              <span className="text-gray-400 text-sm">Next Availability</span>
              <span className="text-white font-semibold text-sm">{ room.availability.nextAvailability}</span>
            </div>
                ...
                <div key={idx} className="space-y-0.5">
                  <div className="flex justify-between items-start">
                    <div className="flex-1">
                      <span className={`text-sm ${isMainPrice ? 'text-gray-300 font-medium' : 'text-gray-400'}`}>
                        {price.type}
                    ...
                    <span className={`font-semibold text-sm whitespace-nowrap ml-2 ${isMainPrice ? 'text-orange-500 font-bold text-base' : 'text-white'}`}>
          ...
          {room.amenities && room.amenities.length > 0 && (
            <div className="mb-3">
              <h5 className="text-white font-semibold text-sm mb-2">Room Amenities</h5>
      ...
      <Button 
              className="w-full bg-linear-to-r from-orange-500 to-orange-600 hover:from-orange-600 hover:to-orange-700 text-white shadow-lg group-hover:shadow-orange-500/30 transition-all duration-300 text-sm h-9"
