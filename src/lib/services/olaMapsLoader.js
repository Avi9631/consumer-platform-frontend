...
export const loadOlaMapsSDK = () => {
  return new Promise((resolve, reject) => {
    // Already loaded
    if (isLoaded && typeof window !== 'undefined' && window.OlaMaps) {
      resolve();
      return;
    }
    ...
    // Check if we're in server-side rendering
    if (typeof window === 'undefined') {
      reject(new Error('Ola Maps SDK can only be loaded in the browser'));
      return;
    }
    ...
    if (!apiKey) {
      const error = new Error('Ola Maps API key not found. Please add NEXT_PUBLIC_OLA_MAPS_API_KEY to your .env.local file');
      loadError = error;
      isLoading = false;
...
export const isOlaMapsLoaded = () => {
  return isLoaded && typeof window !== 'undefined' && window.OlaMaps;
};
